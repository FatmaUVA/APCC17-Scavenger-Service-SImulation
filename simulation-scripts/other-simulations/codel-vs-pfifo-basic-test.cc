...
int main (int argc, char *argv[])
{
  ...
  uint32_t queueSize = 2;      //in packets
...
//      tchCoDel.Install (devicesBottleneckLink);
   ...
   Ptr<NetDevice> nd1 = devicesBottleneckLink.Get (0);
  ...
  if (queueDiscType.compare ("PfifoFast") == 0 || queueDiscType.compare ("CoDel") == 0)
    {
      std::cout << std::endl << "*** TC Layer statistics ***" << std::endl;
      std::cout << "  Packets dropped by the TC layer: " << q1->GetTotalDroppedPackets () << std::endl;
  ...
  flowHelper.SerializeToXmlFile ("pfifo-vs-codel.flowmonitor", true, true);
