...
// PPBP model is used for the background traffic
//
...
//    std::string filename = "/users/fha6np/ns-workspace/ns-allinone-3.26/ns-3.26/60s-1514MTU-parsed-ts-len-1G.txt";
    std::string filename = "/users/fha6np/ns-allinone-3.26/ns-3.26/60s-1514MTU-parsed-ts-len-1G.txt";
//    std::string filename = "/users/fha6np/ns-workspace/ns-allinone-3.26/ns-3.26/60s-64KMTU-ts-changed.txt";
//Qdisc setup:
  ...
  cmd.AddValue ("noSender", "Number of EF senders",noSender);
...
//TCP settings
  if( caseX == "A")
    tcpBuff = 5000000;
//  Config::SetDefault ("ns3::TcpSocket::SegmentSize", UintegerValue (63948));
...
// Random variable stream for file size and interarrival time
  ...
  uint16_t port = 50000;
  uint8_t tos; // 0x08 band 2(low prio) and 0x10 band 0(high prio)


//  The first node generate traffic based on PPBP
//
// Create PPBP application and install it on sender1
//
    ...
    PPBPHelper bkg_source ("ns3::UdpSocketFactory", destAddress2);
    bkg_source.SetAttribute ("BurstIntensity",  StringValue (mean_rate));
    bkg_source.SetAttribute ("MeanBurstArrivals",  DoubleValue (mean_flow_arrival));
    ApplicationContainer backgroundApp = bkg_source.Install (sendHostsContainer.Get (0));
    backgroundApp.Start (Seconds (0.1));
    backgroundApp.Stop (Seconds (simTime));

//
// Create background sink  application receiver and install it on recv1
//
...
//================================================================================
  ...
  if (eMean == 0.6)
  {
     uFileName = "5-26-2017-results/eMean-0.6/utilization-case" + caseX + "-" + service + "-eMean-0.6" + std::to_string(randSeed) + ".txt";
     flowFileName = "5-26-2017-results/eMean-0.6/parsed-case" + caseX + "-" + service + "-eMean-0.6" + std::to_string(randSeed) + ".txt";
     caidaFileName = "5-26-2017-results/eMean-0.6/bkg-loss-case" + caseX + "-" + service + "-eMean-0.6.txt";
  }
  else
  {
    uFileName = log_dir+ "/eMean-" + std::to_string(eMean) + "/utilization-case" + caseX + "-" + service + "-eMean-" + std::to_string(eMean) + "-bRate-" + mean_rate + "-seed-"+ std::to_string(randSeed) +".txt";
    flowFileName = log_dir + "/eMean-" + std::to_string(eMean) + "/parsed-case" + caseX + "-" + service + "-eMean-" + std::to_string(eMean) + "-bRate-" + mean_rate + "-seed-"+ std::to_string(randSeed) +".txt";
    caidaFileName = log_dir+ "/eMean-" + std::to_string(eMean) + "/bkg-loss-case" + caseX + "-" + service + "-eMean-" + std::to_string(eMean) + "-bRate-" + mean_rate + "-seed-"+ std::to_string(randSeed) + ".txt";
  }
 ...
 /*   if (systemId == 2)
      hostLink.EnablePcap("send-left", sendHostsDevices, true);
    if (systemId == 3)
      hostLink.EnablePcap("recv-right", recvHostsDevices, true); 
*/
  }
...
//  Simulator::Schedule(Seconds(1),&ThroughputMonitor, &fmHelper, allMon);
  Simulator::Schedule(Seconds(simTime-0.000000001),&LinkUtilization, rQ1, uStream);
